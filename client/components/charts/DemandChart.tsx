...
function buildForecast(data: DemandPoint[], days = 3) {
  ...
  data.forEach((d) => {
    out.push({ name: d.day, actual: d.sold, forecast: Math.round(movingAverage(soldSeries, 3)) });
  });
  ...
  for (let i = 0; i < days; i++) {
    ...
    out.push({ name: names[(names.indexOf(data[data.length - 1]?.day ?? "Mon") + i + 1) % names.length], forecast: next });
...
export function DemandChart({ data, forecastDays = 3 }: Props) {
  const series = useMemo(() => buildForecast(data, forecastDays), [data, forecastDays]);

  return (
    <div className="w-full h-64">
      <ResponsiveContainer>
        <AreaChart data={series} margin={{ left: 6, right: 6, top: 8, bottom: 0 }}>
          <defs>
          ...
          <YAxis tick={{ fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} width={32} />
          <Tooltip
            contentStyle={{ background: "hsl(var(--popover))", border: "1px solid hsl(var(--border))", color: "hsl(var(--popover-foreground))" }}
          />
          <Area type="monotone" dataKey="actual" name="Actual" stroke="hsl(var(--primary))" fill="url(#actual)" strokeWidth={2} />
          <Area type="monotone" dataKey="forecast" name="Forecast" stroke="hsl(var(--accent-foreground))" fill="url(#forecast)" strokeDasharray="5 5" strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
